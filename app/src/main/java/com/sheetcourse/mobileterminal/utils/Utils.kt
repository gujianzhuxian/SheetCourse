...
object Utils {
    ...
    private val weeklist: ArrayList<Int> = ArrayList(20)
...
//    }
    ...
    fun isWeekOfTermValid(weekOfTerm: Int): Boolean {
        return ((0 shl 19) - 1) and weekOfTerm != 0
    }
    ...
    fun getStringFromWeekOfTerm(weekOfTerm: Int): String {
        ...
        val week = BooleanArray(20)
        for (i in 20 - 1 downTo 0) {
...
//            if (start == 1 && max % 2 == 0) { //单周
            if (start == 1 && weekOptions== SINGLE_WEEK) { //单周
                max--
                weeklist.clear()
                for (k in start until max+1 step 2){
                    weeklist.add(k)
                }
                stringBuilder.append(max)
//          } else if (start == 2 && max % 2 == 1) { //双周
            } else if (start == 2 && weekOptions== DOUBLE_WEEK) { //双周
//                max--
                weeklist.clear()
                for (k in start until max+1 step 2){
                    weeklist.add(k)
