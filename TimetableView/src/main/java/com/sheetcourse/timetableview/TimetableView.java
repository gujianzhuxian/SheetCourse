//TimetableView控件设计的思想：对LinerLayout布局进行设计；
// 先对课程表的各种属性包括星期栏、侧边栏、旗标栏进行设计；
// 然后在另一个类中，对布局属性进行初始化，给相应布局设计自定义的属性，计算面板的坐标（假定）及布局；
//      面板设计思想：7个layout数组，每个layout数组中填充每个课程项的view，view该填充到那个数组
// 接着，在另一个类中，将数据源计算规则，并填充到面板中；
// 最后，本控件实现课表页面属性的初始化与数据的面板填充

package com.sheetcourse.timetableview;
...
import com.sheetcourse.timetableview.listener.OnConfigHandleAdapter;
import com.sheetcourse.timetableview.listener.OnDateBuildAapter;
import com.sheetcourse.timetableview.listener.OnFlaglayoutClickAdapter;
import com.sheetcourse.timetableview.listener.OnItemBuildAdapter;
import com.sheetcourse.timetableview.listener.OnItemClickAdapter;
import com.sheetcourse.timetableview.listener.OnItemLongClickAdapter;
import com.sheetcourse.timetableview.listener.OnScrollViewBuildAdapter;
import com.sheetcourse.timetableview.listener.OnSlideBuildAdapter;
import com.sheetcourse.timetableview.listener.OnSpaceItemClickAdapter;
import com.sheetcourse.timetableview.listener.OnWeekChangedAdapter;
import com.sheetcourse.timetableview.model.Schedule;
import com.sheetcourse.timetableview.model.ScheduleColorPool;
import com.sheetcourse.timetableview.model.ScheduleEnable;
import com.sheetcourse.timetableview.model.ScheduleSupport;
import com.sheetcourse.timetableview.operater.AbsOperater;
import com.sheetcourse.timetableview.operater.SimpleOperater;
import com.sheetcourse.timetableview.utils.ScreenUtils;
