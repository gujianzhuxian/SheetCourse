...
import com.sheetcourse.timetableview.model.ScheduleConfig;
import com.sheetcourse.timetableview.model.ScheduleSupport;
import com.sheetcourse.timetableview.utils.ColorUtils;
...
public class SimpleOperater extends AbsOperater{
    ...
    public void init(Context context, AttributeSet attrs, TimetableView view) {
        ...
        dateLayout = mView.findViewById(R.id.id_datelayout);//表头
    ...
    private void addToLayout(LinearLayout layout, final List<Schedule> data, int curWeek) {
        ...
        for (int i = 0; i < filter.size(); i++) {
            ...
            View view = newItemView(data,filter, subject, pre, i, curWeek);//构建课程项
            if (view != null) {
                layout.addView(view);
                pre = subject;
            }
    ...
    public void startTodo(){
        ...
        for (int i = 0; i < panels.length; i++) {
            ...
            addToLayout(panels[i], data[i], mView.curWeek());//将数据data添加到layout的布局上
        }
    }
    ...
    public void showView() {
        ...
        checkConfig();//检查本地配置情况
        replaceScrollView();//设计了面板跟空白布局点击监听
