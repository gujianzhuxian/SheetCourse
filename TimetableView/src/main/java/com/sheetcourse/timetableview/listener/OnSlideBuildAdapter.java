...
public class OnSlideBuildAdapter implements ISchedule.OnSlideBuildListener {
     ...
     * @return
     */
    public OnSlideBuildAdapter setTimes(String[] times,String[] endtimes) {
        this.times = times;
    ...
    public View getView(int pos, LayoutInflater inflater, int itemHeight, int marTop) {
        ...
        TextView endtimeTextView=view.findViewById(R.id.item_slide_endtime);
        ...
        if(endtimes==null) endtimeTextView.setText("");
        if(endtimes!=null&&pos>=0&&pos<endtimes.length){
            endtimeTextView.setText(endtimes[pos]);
            endtimeTextView.setTextColor(timeTextColor);
            endtimeTextView.setTextSize(timeTextSize);
        }
        return view;
    }
